...
export class ReactStatusCode implements InfoExtractor {
  browser: Browser;
  db: Storage;
  static baseUrl = "https://react.statuscode.com";
  ...
  static canHandle(url: string) {
    return url.startsWith(ReactStatusCode.baseUrl);
  }
  ...
  async updateUrl(url: string): Promise<void> {
    ...
    const content = await extractContent(page, ReactStatusCode.baseUrl);
  ...
  async update() {
    ...
    await page.goto(`${ReactStatusCode.baseUrl}/issues`);
    ...
    for (const issue of issues) {
      ...
      try {
        ...
        const content = await extractContent(page, ReactStatusCode.baseUrl);
        this.db.saveContent("reactstatus", {
          url,
          date,
          content,
        });
